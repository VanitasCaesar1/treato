...
export default function DermatologySection({ dermatologyData, onChange, diagnosisId, onSaved }) {
  ...
  // Fetch existing data when component mounts or diagnosisId changes
  useEffect(() => {
    const fetchExistingData = async () => {
      ...
      setLoading(true);
      try {
        const response = await fetch(`/api/diagnosis/${diagnosisId}/dermatology`);
        
        if (response.ok) {
          const result = await response.json();
          console.log("Fetched dermatology data:", result);
          
          // Handle the wrapped response structure from your backend
          const dermatologyData = result.data || result;
          
          if (dermatologyData && Object.keys(dermatologyData).length > 0) {
            // Merge with DEFAULT_FORM to ensure all fields are present
            const mergedData = {
              ...DEFAULT_FORM,
              ...dermatologyData,
              appointment_id: diagnosisId
            };
            console.log("Setting merged data:", mergedData);
            setData(mergedData);
            onChange?.(mergedData);
          } else {
            // No existing data, use default with appointment_id
            const defaultData = {
              ...DEFAULT_FORM,
              appointment_id: diagnosisId
            };
          ...
          // No existing data found, use default
          console.log("No existing dermatology data found, using defaults");
          const defaultData = {
            ...DEFAULT_FORM,
            appointment_id: diagnosisId
          };
          ...
          console.error("Failed to fetch dermatology data:", response.status);
        }
      } catch (error) {
        console.error("Error fetching dermatology data:", error);
        // Still set default data with appointment_id on error
        const defaultData = {
          ...DEFAULT_FORM,
          appointment_id: diagnosisId
        };
    ...
    try {
      ...
      try {
        ...
        if (checkRes.ok) {
          ...
          // Check if data exists in the response (handle wrapped response)
          const actualData = checkData.data || checkData;
          if (actualData && Object.keys(actualData).length > 0 && !actualData.error) {
            exists = true;
          }
        }
      } catch (e) {
    ...
    return (
      <div className="max-w-6xl mx-auto p-6 bg-gray-50 rounded-lg">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading dermatology data...</p>
        </div>
      </div>
    );
  }
      ...
      {activeTab === 'documentation' && (
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <div className="mt-4">
              <label className="block text-sm font-medium mb-2">Diagnostic Procedures</label>
            ...
            <div className="mt-4">
              <label className="block text-sm font-medium mb-2">Physical Exam Notes</label>
              <Textarea
                placeholder="Additional physical examination findings..."
                value={data?.physical_exam_notes || ""}
            ...
            <div className="mt-4">
              ...
              <Textarea
                placeholder="Additional clinical assessment notes..."
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div>
                ...
                <Textarea
                  placeholder="When should patient return? What to monitor..."
            ...
            <div className="mt-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                />
                <label htmlFor="referral_needed" className="text-sm font-medium">
                  Referral Required
                </label>
              </div>
