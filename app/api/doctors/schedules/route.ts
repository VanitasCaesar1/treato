...
export async function POST(req: NextRequest) {
  try {
    ...
    // FIX: Access the correct properties returned from the backend
    // The backend returns doctorID, organizationID, and weekday directly
    const compositeId = `${response.data.doctorID || data.doctorID}_${response.data.weekday || data.weekday}_${response.data.organizationID || organizationId}`;
